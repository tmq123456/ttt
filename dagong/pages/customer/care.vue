	...
	<view class="content">
		...
		<view class="goods-list">
			<view 
				...
				class="goods-item">
				<text class="title clamp" @click="navToDetailPage(item)">客户1</text>
				<view class="price-box">
					<text>销售归属：小王</text>
				</view>
				<view class="price-box" style="margin-top: 10px;">
					<text @click="callphone()"><img src="/static/phone.jpg" style="width: 18px;height: 18px;"/></text>
					<text @click="confirm()">确认处理</text>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async loadCateList(fid, sid){
				let list = await this.$api.json('cateList6');
			...
			async loadCateList1(fid, sid){	
				let list = await this.$api.json('cateList7');
				let cateList = list.filter(item=>item.pid == fid);
			...
			async loadCateList2(fid, sid){
				let list = await this.$api.json('cateList8');
				let cateList = list.filter(item=>item.pid == fid);
