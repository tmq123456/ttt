			...
			<view class="cate-content" @click.stop.prevent="stopPrevent" @touchmove.stop.prevent="stopPrevent">
				<scroll-view scroll-y class="cate-list">
					<view v-for="item in cateList" :key="item.id">
						<view class="cate-item b-b two" @click="changeCate(item.id,item.realname)">{{item.realname}}</view>
					</view>
				</scroll-view>
			</view>
		</view>
			...
			<view class="cate-content" @click.stop.prevent="stopPrevent" @touchmove.stop.prevent="stopPrevent">
				<scroll-view scroll-y class="cate-list">
					<view v-for="item in cateList1" :key="item.id">
						<view class="cate-item b-b two" @click="changeCate1(item.id,item.brand)">{{item.brand}}{{item.brand1}}{{item.brand_chexing}}</view>
					</view>
				</scroll-view>
			</view>
		</view>
...
<script>
	export default {
		...
		onLoad(option){
			...
			_self.merchant_id=userinfo.merchant_id;
			if(id){
			_self.id=id;
			uni.request({
				url: 'http://dg.51jump.cn/merapi/v1/car/car-member/info', 
				data: {
				...
				success: function (res) {
					if(res.data.code=== 200){
						...
						_self.bianhao1=res.data.data.sex;
						_self.bianhao2=res.data.data.is_key;
						_self.customerData.id=id;
						_self.color_id=res.data.data.mer_id;
						_self.type3Index=res.data.data.from;
						_self.car_id=res.data.data.intention;
						_self.typeIndex=res.data.data.talk_type;
						_self.type2Index=res.data.data.level;
					}else{
		...
		methods: {
			...
			submit(){
				...
				uni.request({
				    url: 'http://dg.51jump.cn/merapi/v1/car/car-member/update?access-token='+this.access_token+'&id='+this.id, //仅为示例，并非真实接口地址。
